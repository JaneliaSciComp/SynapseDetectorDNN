...
######## Case 2
    ...
    bsub -J "cp_input${RANDIDX}" -n 3 -o /dev/null "cp $INPUT_DIR/*.h5 $OUTPUT_DIR"
    ...
    for (( ROW=0; ROW<$NUM_ROW; ROW++ )); do
        for (( COL=0; COL<$NUM_COL; COL++ )); do
            for (( VOL=0; VOL<$NUM_VOL; VOL++ )); do
                ...
                "singularity run -B /misc/local/matlab-2018b/ -B /groups/dickson/dicksonlab/ -B /nrs/dickson/ $SCRIPT_DIR/singularity_for_2D_synapse2020_6.simg postprocess_cpu.py -i $INPUT_IMG -l $MIN_ROW,$MIN_COL,$MIN_VOL,$MAX_ROW,$MAX_COL,$MAX_VOL -m $OUTPUT_DIR/MASK/slices_to_volume.h5 -t $THRESHOLD -p $PERCENTAGE"
            done
        done
    done
    if [[ $TO_TIFF == "true" ]]; then
        bsub -w "done("post${RANDIDX}_*")" -J "hdftotif${RANDIDX}" -n 3 -o $OUTPUT_DIR/result_hdf2tif.log \
        "singularity run -B /groups/dickson/dicksonlab/ -B /nrs/dickson/ $SCRIPT_DIR/singularity_for_2D_synapse2020_6.simg h5_to_tif.py -i $INPUT_IMG -o $OUTPUT_DIR/tif_results"
    fi
