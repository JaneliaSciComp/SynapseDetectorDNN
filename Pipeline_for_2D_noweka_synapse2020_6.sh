...
SCRIPT_DIR=/groups/dickson/home/lillvisj/model_DNN/singularity_build_test
...
"singularity run -B /groups/dickson/dicksonlab/ -B /nrs/dickson/ $SCRIPT_DIR/singularity_for_2D_synapse2020_6.simg tif_to_h5.py -i $INPUT_DIR -o $OUTPUT_DIR"
...
if [[ $MASK_DIR != "" ]]; then
    if [[ `ls $MASK_DIR/*.tif | wc -l` != 0 ]]; then 
        ...
        "singularity run -B /groups/dickson/dicksonlab/ -B /nrs/dickson/ $SCRIPT_DIR/singularity_for_2D_synapse2020_6.simg tif_to_h5.py -i $MASK_DIR -o $OUTPUT_DIR/MASK"
    else
...
for (( ROW=0; ROW<$NUM_ROW; ROW++ )); do
    for (( COL=0; COL<$NUM_COL; COL++ )); do
        for (( VOL=0; VOL<$NUM_VOL; VOL++ )); do
            ...
            "singularity run --nv -B /misc/local/matlab-2018b/ -B /groups/dickson/dicksonlab/ -B /nrs/dickson/ $SCRIPT_DIR/singularity_for_2D_synapse2020_6.simg unet_gpu.py -i $OUTPUT_DIR/slices_to_volume.h5 -l $MIN_ROW,$MIN_COL,$MIN_VOL,$MAX_ROW,$MAX_COL,$MAX_VOL"
            if [[ $MASK_DIR != "" ]]; then
                bsub -w "done("unet${RANDIDX}_$IDX")" -J "post${RANDIDX}_$IDX" -n 3 -o $OUTPUT_DIR/post_${MIN_ROW}_${MIN_COL}_${MIN_VOL}.log \
                "singularity run -B /misc/local/matlab-2018b/ -B /groups/dickson/dicksonlab/ -B /nrs/dickson/ $SCRIPT_DIR/singularity_for_2D_synapse2020_6.simg postprocess_cpu.py -i $OUTPUT_DIR/slices_to_volume.h5 -l $MIN_ROW,$MIN_COL,$MIN_VOL,$MAX_ROW,$MAX_COL,$MAX_VOL -m $OUTPUT_DIR/MASK/slices_to_volume.h5 -t $THRESHOLD -p $PERCENTAGE"
            else
                bsub -w "done("unet${RANDIDX}_$IDX")" -J "post${RANDIDX}_$IDX" -n 3 -o $OUTPUT_DIR/post_${MIN_ROW}_${MIN_COL}_${MIN_VOL}.log \
                "singularity run -B /misc/local/matlab-2018b/ -B /groups/dickson/dicksonlab/ -B /nrs/dickson/ $SCRIPT_DIR/singularity_for_2D_synapse2020_6.simg postprocess_cpu.py -i $OUTPUT_DIR/slices_to_volume.h5 -l $MIN_ROW,$MIN_COL,$MIN_VOL,$MAX_ROW,$MAX_COL,$MAX_VOL -t $THRESHOLD -p $PERCENTAGE"
            fi
        done
    done
done
...
if [[ $TO_TIFF == "true" ]]; then
    ...
    "singularity run -B /groups/dickson/dicksonlab/ -B /nrs/dickson/ $SCRIPT_DIR/singularity_for_2D_synapse2020_6.simg h5_to_tif.py -i $OUTPUT_DIR/slices_to_volume.h5 -o $OUTPUT_DIR/tif_results"
fi
