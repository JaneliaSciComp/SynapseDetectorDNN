...
if [[ $1 == "-unet" ]]; then
    ...
    "singularity run -B /groups/dickson/dicksonlab/ -B /nrs/dickson/ $SINGULARITY_IMAGE tif_to_h5.py -i $INPUT_DIR -o $OUTPUT_DIR"
    ...
    for (( ROW=0; ROW<$NUM_ROW; ROW++ )); do
        for (( COL=0; COL<$NUM_COL; COL++ )); do
            for (( VOL=0; VOL<$NUM_VOL; VOL++ )); do
                ...
                "singularity run --nv -B /misc/local/matlab-2018b/ -B /groups/dickson/dicksonlab/ -B /nrs/dickson/ $SINGULARITY_IMAGE unet_gpu.py -i $OUTPUT_DIR/slices_to_volume.h5 -l $MIN_ROW,$MIN_COL,$MIN_VOL,$MAX_ROW,$MAX_COL,$MAX_VOL"
            done
        done
    done
...
######## Case 2
    ...
    "singularity run -B /groups/dickson/dicksonlab/ -B /nrs/dickson/ $SINGULARITY_IMAGE tif_to_h5.py -i $MASK_DIR -o $OUTPUT_DIR/MASK"
    # Get the dimension of image
    ...
    for (( ROW=0; ROW<$NUM_ROW; ROW++ )); do
        for (( COL=0; COL<$NUM_COL; COL++ )); do
            for (( VOL=0; VOL<$NUM_VOL; VOL++ )); do
                ...
                "singularity run -B /misc/local/matlab-2018b/ -B /groups/dickson/dicksonlab/ -B /nrs/dickson/ $SINGULARITY_IMAGE postprocess_cpu.py -i $INPUT_IMG -l $MIN_ROW,$MIN_COL,$MIN_VOL,$MAX_ROW,$MAX_COL,$MAX_VOL -m $OUTPUT_DIR/MASK/slices_to_volume.h5 -t $THRESHOLD -p $PERCENTAGE"
            done
        done
    done
    if [[ $TO_TIFF == "true" ]]; then
        bsub -w "done("post${RANDIDX}_*")" -J "hdftotif${RANDIDX}" -n 3 -o $OUTPUT_DIR/result_hdf2tif.log \
        "singularity run -B /groups/dickson/dicksonlab/ -B /nrs/dickson/ $SINGULARITY_IMAGE h5_to_tif.py -i $INPUT_IMG -o $OUTPUT_DIR/tif_results"
    fi
